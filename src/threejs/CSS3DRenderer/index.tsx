...
export function CSS3DRendererView() {
	...
	const camreaRef = React.useRef<THREE.PerspectiveCamera>()
	...
	// 宽高自适应
	React.useEffect(() => {
		if (!camreaRef.current || !rendererRef.current) return

		rendererRef.current.setSize(wh.width, wh.height)
		camreaRef.current.aspect = wh.width / wh.height
	}, [wh.height, wh.width])
